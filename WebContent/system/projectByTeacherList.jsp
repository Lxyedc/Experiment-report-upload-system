...
<body>
 <div class="panel admin-panel">
   ...
   <div class="padding border-bottom">
  	 <form action="reportManager?pageNum=1&type=selAllProject&id=<%=user.getId() %>" method="post">
     ...
     <% 
       ...
       <tr>
         ...
         <td><div class="button-group">
        	   <a class="button border-main" href="<%=path %>/reportManager?id=<%=p.getId() %>&pageNum=1&type=selReport"><span class="icon-edit"></span> 详情 </a>
         	 </div>
         </td>
        </tr>
      <%}
        ...
        <div class="pagelist"> 
	        <span class="current">当前为第<%=pageNum %>页,共<%=totalPage %>页</span>
	        	<% if(Integer.valueOf(pageNum) > 1){ %>
					<a href="<%=path %>/reportManager?pageNum=<%=Integer.valueOf(pageNum)-1 %>&type=selAllProject&searchName=<%=searchName %>&id=<%=user.getId() %>">上一页</a>
				<%}else{ %>
					<a href="#">上一页 </a>
				<%} %>
				<% if(start <= totalPage){%> 
					<a href="<%=path %>/reportManager?pageNum=<%=start %>&type=selAllProject&searchName=<%=searchName %>&id=<%=user.getId() %>">下一页</a> 
					<a href="<%=path %>/reportManager?pageNum=<%=totalPage %>&type=selAllProject&searchName=<%=searchName %>&id=<%=user.getId() %>">尾页</a>
				<%}else{ %>
					<a href="#">下一页</a>  
					<a href="#">尾页 </a>
				<% }%>
