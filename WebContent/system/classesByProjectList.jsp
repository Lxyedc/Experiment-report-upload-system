...
<body>
 <div class="panel admin-panel">
   ...
   <div class="padding border-bottom">
  	 <form action="classesManager?pageNum=1&type=selClasses&id=<%=user.getId() %>" method="post">
     ...
     <% 
       ...
       <tr>
         ...
         <td><div class="button-group">
        	   <a class="button border-main" href="<%=path %>/projectManager?id=<%=c.getId() %>&pageNum=1&type=info"><span class="icon-edit"></span> 详情 </a>
         	   <a class="button border-red" href="<%=path %>/classesManager?id=<%=c.getId() %>&searchName=<%=searchName %>&type=delClasses&pageNum=<%=pageNum%>"><span class="icon-trash-o"></span> 删除</a>
 		     </div>
         </td>
        </tr>
      <%}
        ...
        <div class="pagelist"> 
	        <span class="current">当前为第<%=pageNum %>页,共<%=totalPage %>页</span>
	        	<% if(Integer.valueOf(pageNum) > 1){ %>
					<a href="<%=path %>/classesManager?pageNum=<%=Integer.valueOf(pageNum)-1 %>&type=selClasses&searchName=<%=searchName %>&id=<%=user.getId() %>">上一页</a>
				<%}else{ %>
					<a href="#">上一页 </a>
				<%} %>
				<% if(start <= totalPage){%> 
					<a href="<%=path %>/classesManager?pageNum=<%=start %>&type=selClasses&searchName=<%=searchName %>&id=<%=user.getId() %>">下一页</a> 
					<a href="<%=path %>/classesManager?pageNum=<%=totalPage %>&type=selClasses&searchName=<%=searchName %>&id=<%=user.getId() %>">尾页</a>
				<%}else{ %>
					<a href="#">下一页</a>  
					<a href="#">尾页 </a>
				<% }%>
